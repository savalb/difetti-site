...
export function ContattiClient() {
  ...
  async function handleSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    const formspreeId = process.env.NEXT_PUBLIC_FORMSPREE_ID || 'FORM_ID';
    ...
    try {
      ...
      if (res.ok) {
        setStatus('ok');
        track('form_submit_success', { form: 'contatti', tipo: data.get('tipo')?.toString() });
        form.reset();
      } else {
        setStatus('error');
        track('form_submit_error', { form: 'contatti' });
      }
    } catch (err) {
      setStatus('error');
      track('form_submit_error', { form: 'contatti', exception: String(err) });
    }
  }
          ...
          <div className={styles.info}>
            <div className={styles.infoBlock}>
              ...
              <a
                ...
                onClick={() => track('whatsapp_click', { location: 'contatti' })}
              >
